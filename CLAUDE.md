...
- **Config-relative db_path**: When loading a config via `--config`, relative `db_path` values are resolved against the config file's directory, not the process cwd. This ensures the proxy writes to the correct database when launched by MCP clients (which may use a different working directory).
